...
import { PhotoFlowStackParamList, VideoSearchResponse } from '../../types/photo';

type Props = NativeStackScreenProps<PhotoFlowStackParamList, 'VideoRecipe'>;

// Web-only iframe. On native this component is never rendered (Platform check below).
function WebVideoEmbed({ embedUrl }: { embedUrl: string }) {
  // On web, React Native Web renders unknown elements via createElement fallback,
  // but the safest cross-runtime approach is to use a div with dangerouslySetInnerHTML
  // via React.createElement. We instead rely on the built-in <iframe> support from
  // react-native-web when the component is rendered through a raw element.
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.header}>
          <Text style={styles.eyebrow}>STEP 4 / 4 · VIDEO RECIPE</Text>
        ...
        <View style={styles.videoWrap}>
          ...
          {!loading && !error && video && Platform.OS === 'web' && (
            <WebVideoEmbed embedUrl={video.embedUrl} />
          )}
          {!loading && !error && video && Platform.OS !== 'web' && (
