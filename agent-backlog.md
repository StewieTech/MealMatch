...
- 2026-04-25: Replaced `Home` landing screen with new `/multi` input chooser (`MultiScreen.tsx`) — three Ionicons tiles (camera / mic / create) routing to PhotoFlow, VoiceFlow, and VoiceFlow > ConfirmIngredients (empty ingredients) for manual entry. Updated `RootStackParamList` to use `NavigatorScreenParams` so nested navigation typechecks. Deleted `HomeScreen.tsx`. Updated `App.test.tsx` to assert chooser tiles by accessibility labels. 13/13 tests pass; typecheck clean; no new deps (uses bundled `@expo/vector-icons`).
- 2026-04-26: Restored missing `mobile/app/screens/MultiScreen.tsx` after a regression where the file was never committed but its consumers (`App.tsx`, `App.test.tsx`, `HomeScreen.tsx` deletion) remained, causing Metro "Unable to resolve ./app/screens/MultiScreen". Recreated the screen with required labels/navigation and recommitted the full consumer set to prevent recurrence.
- 2026-04-26: Fixed Metro web bundle regression by creating missing `mobile/app/components/recipe/CombinedRecipeView.tsx` that `VideoRecipeScreen.tsx` and `RecipeDetailScreen.tsx` already imported, completing the shared video+recipe view refactor. Hardened `scripts/release-prod.ps1` with branch-switch warning, `-ReturnToOriginalBranch` opt-in behavior, and `git reflog`/`git checkout <original-branch>` recovery hints across throw paths + `finally`.
- 2026-04-26: Finished the Confirm Ingredients shared-view extraction by creating `mobile/app/components/shared/ConfirmIngredientsView.tsx` and wiring the existing voice/photo confirm screens to a real shared component (IngredientEditor + FilterBar + submit CTA), resolving the missing-module import regression and restoring clean mobile typecheck/tests.
- 2026-04-26: Polished `MultiScreen.tsx` landing page with outcome stepper hero (Ingredients › Recipe › Video), horizontal row of ghost-outlined input cards (Photo / Voice / Type) with one-line hints, and updated accessibility labels to be outcome-oriented. All styling uses theme tokens; no new dependencies. Updated `App.test.tsx` to match new copy. 13/13 tests pass; typecheck clean.
- 2026-04-26: Restyled `MultiScreen.tsx` to vertical layout with black filled circles (72x72, theme.colors.textPrimary) and white Ionicons size 30, removed pressedCard state, added ScrollView for short-screen fallback, centered content column with maxWidth 360, and press feedback (scale 0.96 + opacity 0.85). All styling uses theme tokens; no new hex; navigation targets unchanged. 13/13 tests pass; typecheck clean.
